defmodule Explorer.Chain.PlatonAppchain.Query do
  ...
  alias Explorer.Chain.PlatonAppchain.{L1Event, L1Execute, L2Event, L2Execute, Commitment, Checkpoint, L2Validator, L2Delegator}
  ...
  def withdrawals(options \\ []) do
    ...
    base_query =
      from(
        ...
        on: l2e.block_number >= c.start_block_number and l2e.block_number <= c.end_block_number,
        select: %{
  ...
  def delegations(address_hash, options \\ []) do
    ...
    base_query =
      from(
        ...
        order_by: [desc: v.validator_hash]
    )
